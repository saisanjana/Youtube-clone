...
import {setSuggestionsData}  from '../Redux/sugestionsSlice'
...
function Header() {
  ...
  const [clickedInSBox, setClickedInSBox] = useState(false);
  ...
  const getSearchResults = async (search) => {
    let data = await fetch(SEARCH_API + search);
    const results = await data.json()
    if(results && results[1]){
      setSearchResults(results[1]);
      dispatch(setSuggestionsData({[search]:results[1]}));
    }
  }
  ...
  return (
    ...
    className='m-2 h-8 text-left inline-block cursor-pointer'
    ...
    />
    <img
    className='h-7 p-0 inline-block border-2 rounded-r-xl border-gray-300'
    src='https://static.vecteezy.com/system/resources/previews/000/439/746/original/vector-find-icon.jpg'
    />
    </div>
    <img
    className='m-2 col-span-1 h-8'
    ...
    {showSearchResults && <Suggestions suggestionsArray = {searchResults} setShowSearchResults={setShowSearchResults}/>}
    </div>
  )
}
